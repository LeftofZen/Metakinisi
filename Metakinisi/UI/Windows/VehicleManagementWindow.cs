...
namespace Metakinisi.UI
{
	public class VehicleManagementWindow : Window
	{
		...
		public VehicleManagementWindow(Rectangle bounds, string title) : base(bounds, title)
		{
			...
			{
				Items = GameServices.GetService<GridWorld>().GameState.Vehicles,
			...
			{
				GameServices.GetService<GridWorld>().GameState.Vehicles.Add(new Vehicle(new Point3(10, 10, 0), 0.5f, $"vehicle{GameServices.GetService<GridWorld>().GameState.Vehicles.Count}"));
			});
			...
			var deleteVehicleButton = new Button(squareButton, "Delete", () =>
			{
				...
				{
					_ = GameServices.GetService<GridWorld>().GameState.Vehicles.Remove(v);
				}
			});
		...
		public override void Update(GameTime gameTime)
		{
			...
			vehicleListBox.Items = GameServices.GetService<GridWorld>().GameState.Vehicles;
		}
	}
}
